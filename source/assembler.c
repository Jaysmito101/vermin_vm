...
static void VERMIN__assembler_init() {
  macros.count = 0;
  macros.data =
      (macro_t*)VERMIN_malloc(sizeof(macro_t) * instruction_line_count);
  memset(macros.data, 0, sizeof(macro_t) * instruction_line_count);

  labels.count = 0;
  labels.data =
      (label_t*)VERMIN_malloc(sizeof(label_t) * instruction_line_count);
  memset(labels.data, 0, sizeof(label_t) * instruction_line_count);

  instructions.count = 0;
  instructions.data = (instruction_t*)VERMIN_malloc(sizeof(instruction_t) *
...
static bool VERMIM__assembler_parse_reg(const string_t code, uint8_t* type,
                                        uint32_t* value) {
...
static bool VERMIM__assembler_parse_reg_or_mem_or_const(const string_t code,
                                                        uint8_t* type,
                                                        uint32_t* value) {
  static char temp_buffer[256];
  if (code[0] == 'R')
...
static bool VERMIN__assembler_instruction_p1_to_p2_classify(
    uint8_t p1, uint8_t p2, uint16_t* subparams) {
...
static bool VERMIN__assembler_parse_double_param() {
  ...
  if (!VERMIM__assembler_parse_reg_or_mem_or_const(
          current_line[1], &p1, &current_instruction.params[0]) ||
      !VERMIM__assembler_parse_reg_or_mem_or_const(
          current_line[2], &p2, &current_instruction.params[1])) {
    VERMIN_LOG("Failed to parse param\n");
    return false;
  }
  return VERMIN__assembler_instruction_p1_to_p2_classify(
      p2, p1, &current_instruction.subparams);
}
...
static bool VERMIN__assembler_parse_lshift() {
  return VERMIN__assembler_parse_two_param_instruction(
      VERMIN_INSTRUCTION_LSHIFT);
}

static bool VERMIN__assembler_parse_rshift() {
  return VERMIN__assembler_parse_two_param_instruction(
      VERMIN_INSTRUCTION_RSHIFT);
}
...
static bool VERMIN__assembler_parse_jmp(uint8_t type) {
  ...
  if (strlen(current_line[1]) > 0) {
    if (current_line[1][0] == 'R') {
      ...
      current_instruction.params_count = 1;
      return VERMIM__assembler_parse_reg(current_line[1], &subparams_ui[1],
                                         &current_instruction.params[0]);
      ...
      *((uint64_t*)(&current_instruction.params)) = tmp_strings.count;
      sprintf(tmp_strings.data[tmp_strings.count++], "%s",
              (current_line[1] + 1));
      return true;
    } else if (current_line[1][0] == '&') {
      current_instruction.size *= 2;
      current_instruction.params_count = 1;
      bool success =
          VERMIM__assembler_parse_reg(current_line[1] + 1, &subparams_ui[1],
                                      &current_instruction.params[0]);
      ...
      current_instruction.size *= 3;
      current_instruction.params_count = 2;
      *((uint64_t*)(&current_instruction.params)) =
          (uint64_t)atoll(current_line[1]);
      return true;
    }
  } else {
...
static bool VERMIN__assembler_parse_push() {
  ...
  uint8_t type = 0;
  bool success = VERMIM__assembler_parse_reg_or_mem_or_const(
      current_line[1], &type, &current_instruction.params[0]);
...
static bool VERMIN__assembler_parse_pop() {
  ...
  bool success = VERMIM__assembler_parse_reg_or_mem_or_const(
      current_line[1], &type, &current_instruction.params[0]);
  if (type == VERMIN_TARGET_CONST || type == VERMIN_TARGET_REG_MEM ||
      type == VERMIN_TARGET_MEM || type == VERMIN_TARGET_MEMC) {
...
static bool VERMIN__assembler_parse_label() {
  ...
  if (instructions.count > 0)
    label.offset = instructions.data[instructions.count - 1].offset +
                   instructions.data[instructions.count - 1].size;
  else
    label.offset = 0;
...
static bool VERMIN__assembler_parse_macro() {
  if (!(strlen(current_line[0]) > 0 && strlen(current_line[1]) > 0))
    return false;
...
static bool VERMIN__assembler_parse_data() {
  ...
  if (instructions.count > 0)
    instruction_offset = instructions.data[instructions.count - 1].offset +
                         instructions.data[instructions.count - 1].size;
  label_t label;
  memset(&label, 0, sizeof(label_t));
  sprintf(label.name, "%s", (current_line[0] + 1));
  ...
  if (strncmp(current_line[1], "STR", 3) == 0) {
    if (strlen(current_line[1]) > 3) count = max(1, atoi(current_line[1] + 3));
    size_t length = strlen(current_line[2]);
    if (length > 2) {
      current_line[2][length - 1] = '\0';
      data_size = (length - 1) * count;
      for (int i = 0; i < count; i++)
        memcpy(&data_section[data_section_offset + i * (length - 1)],
               current_line[2] + 1, length - 1);
    }
  } else if (current_line[1][0] == 'I') {
    if (strlen(current_line[1]) > 2) count = max(1, atoi(current_line[1] + 2));
    ...
    switch (individual_data_size) {
      case 1: {
        ...
        for (size_t i = 0; i < count; i++) ((int8_t*)data)[i] = value;
        break;
      }
      case 2: {
        int16_t value = (int16_t)atoi(current_line[2]);
        for (size_t i = 0; i < count; i++) ((int16_t*)data)[i] = value;
        break;
      }
      case 4: {
        int32_t value = (int32_t)atoi(current_line[2]);
        for (size_t i = 0; i < count; i++) ((int32_t*)data)[i] = value;
        break;
      }
      case 8: {
        int64_t value = (int64_t)atoll(current_line[2]);
        for (size_t i = 0; i < count; i++) ((int64_t*)data)[i] = value;
        break;
      }
      default:
  ...
  if (strlen(current_line[0]) > 1) labels.data[labels.count++] = label;
  return true;
}

static bool VERMIN__assembler_parse_current_line(size_t line_number) {
  if (current_line[0][0] == ';' || strlen(current_line[0]) == 0) return true;
  ...
  if (current_parser_type == PARSER_TYPE_MARO)
    ...
    if (instructions.count == 0)
      ...
      current_instruction.offset =
          instructions.data[instructions.count - 1].offset +
          instructions.data[instructions.count - 1].size;
    if (current_line[0][0] == '@')
      success = VERMIN__assembler_parse_label();
    else if (memcmp(current_line[0], "OR", 2) == 0)
      success = VERMIN__assembler_parse_or();
    else if (memcmp(current_line[0], "JE", 2) == 0 ||
             memcmp(current_line[0], "JZ", 2) == 0)
      success = VERMIN__assembler_parse_jmp(VERMIN_JMP_TYPE_JZ);
    else if (memcmp(current_line[0], "JNE", 3) == 0 ||
             memcmp(current_line[0], "JNZ", 3) == 0)
    ...
    else if (memcmp(current_line[0], "SUB", 3) == 0 ||
             memcmp(current_line[0], "CMP", 3) == 0)
    ...
    if (!success) VERMIN_LOG("Error on line %zu\n", (line_number + 1));
    return success;
  }
  return true;
}
...
static bool VERMIN__assembler_place_labels() {
  for (size_t i = 0; i < instructions.count; i++) {
    if (instructions.data[i].id == VERMIN_INSTRUCTION_PUSH ||
        instructions.data[i].id == VERMIN_INSTRUCTION_POP) {
      if (instructions.data[i].subparams == VERMIN_TARGET_MEM) {
        uint64_t label_offset = VERMIN__assembler_find_label_offset(
      ...
      if (subparams_ui[1] == VERMIN_TARGET_MEM) {
        uint64_t* label_name_index_ptr =
            ((uint64_t*)(&instructions.data[i].params[0]));
        uint64_t offset = VERMIN__assembler_find_label_offset(
    ...
    } else if (instructions.data[i].id == VERMIN_INSTRUCTION_XOR ||
               instructions.data[i].id == VERMIN_INSTRUCTION_ADD ||
               instructions.data[i].id == VERMIN_INSTRUCTION_SUB ||
               instructions.data[i].id == VERMIN_INSTRUCTION_MUL ||
               instructions.data[i].id == VERMIN_INSTRUCTION_DIV) {
      if (instructions.data[i].subparams == VERMIN_OP_MEM_TO_REG ||
          instructions.data[i].subparams == VERMIN_OP_MEMC_TO_REG) {
        uint64_t offset = VERMIN__assembler_find_label_offset(
      ...
      if (subparams_ui[0] == VERMIN_OP_MEM_TO_REG ||
          subparams_ui[0] == VERMIN_OP_MEMC_TO_REG) {
        uint64_t offset = VERMIN__assembler_find_label_offset(
            tmp_strings.data[instructions.data[i].params[1]]);
        if (offset == UINT64_MAX) {
...
static bool VERMIN__assembler_tokenize_line(const string_t line_original,
                                            size_t line_number) {
  ...
  for (int i = 0, j = 0; i < word_count && j < 3; i++)
    if (strlen(words[i]) > 0) sprintf(current_line[j++], "%s", words[i]);
...
static bool VERMIN__assembler_prepare_final_binary() {
  ...
  if (instructions.count > 0)
    instruction_size = instructions.data[instructions.count - 1].offset +
                       instructions.data[instructions.count - 1].size;
  ...
  for (int i = 0; i < instructions.count; i++) {
    ...
    if (instructions.data[i].params_count > 0)
      memcpy(&instruction_binary[1], &instructions.data[i].params[0],
             instructions.data[i].size - sizeof(uint32_t));
    memcpy(final_binary + offset, instruction_binary,
           instructions.data[i].size);
...
char* VERMIN_assemle(char* code, size_t* binary_size) {
  ...
  for (i = 0; i < lines_count; i++)
    if (!VERMIN__assembler_tokenize_line(lines[i], i)) break;
  final_binary = NULL;
  *binary_size = 0;
  while (i == lines_count)  // using while insted of if just use break in middle
                            // in case of an error
  {
    bool success = false;
    if (!VERMIN__assembler_place_labels()) break;
    if (!VERMIN__assembler_prepare_final_binary()) break;
    printf("Success!\n");
    break;
  }
