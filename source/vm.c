...
static void print_registors() {
  printf("Registors: [ ");
  for (int i = 0; i < 16; i++) printf("(R%d %lld) ", i, registors[i]);
  printf("]\n");
}
...
static bool VERMIN__vm_execute_instruction_MOV() {
  ...
  switch (op) {
    case VERMIN_OP_REG_TO_REG:
      return VERMIN__vm_set_registor_value(
          current_instruction.pointer[1],
          VERMIN__vm_get_registor_value(current_instruction.pointer[2]));
    case VERMIN_OP_CONST_TO_REG:
      return VERMIN__vm_set_registor_value(
          current_instruction.pointer[1],
          *((int32_t*)&current_instruction.pointer[2]));
    case VERMIN_OP_REG_MEM_TO_REG: {
      uint64_t offset = (uint64_t)VERMIN__vm_get_registor_value(
          current_instruction.pointer[2]);
      char* memory_ptr = &memory[offset];
      size_t reg_size =
          VERMIN__vm_set_registor_size(current_instruction.pointer[1]);
      return VERMIN__vm_set_registor_value(
          current_instruction.pointer[1],
          VERMIN__vm_as_number(memory_ptr, reg_size));
    }
    ...
    case VERMIN_OP_REG_TO_REG_MEM: {
      ...
      char* memory_ptr = &memory[offset];
      size_t reg_size =
          VERMIN__vm_set_registor_size(current_instruction.pointer[2]);
...
static bool VERMIN__vm_load_two_params_reg_custom(int64_t* value_1,
                                                  int64_t* value_2) {
  *value_1 = VERMIN__vm_get_registor_value(current_instruction.pointer[1]);
  *value_2 = 0;
  switch (current_instruction.subparams) {
    case VERMIN_OP_REG_TO_REG:
      *value_2 = VERMIN__vm_get_registor_value(current_instruction.pointer[2]);
      break;
    case VERMIN_OP_CONST_TO_REG:
      *value_2 = (int64_t)(*(int32_t*)(&current_instruction.pointer[2]));
      break;
    case VERMIN_OP_REG_MEM_TO_REG:
      *value_2 = (VERMIN__vm_as_number(
          &memory[VERMIN__vm_get_registor_value(
              current_instruction.pointer[2])],
          VERMIN__vm_set_registor_size(current_instruction.pointer[1])));
      break;
    case VERMIN_OP_MEM_TO_REG:
      *value_2 = VERMIN__vm_as_number(
          &memory[current_instruction.pointer[2]],
          VERMIN__vm_set_registor_size(current_instruction.pointer[1]));
      break;
    default:
...
static bool VERMIN__vm_execute_instruction_INC() {
  ...
  return VERMIN__vm_set_registor_value(current_instruction.pointer[1],
                                       value_1 + 1);
}
...
static bool VERMIN__vm_execute_instruction_XOR() {
  ...
  switch (current_instruction.subparams) {
    case VERMIN_OP_REG_TO_REG:
      ...
      break;
    case VERMIN_OP_CONST_TO_REG:
      value_2 = (int64_t)(current_instruction.pointer[2]);
      break;
    case VERMIN_OP_REG_MEM_TO_REG:
      value_2 = (VERMIN__vm_as_number(
          &memory[VERMIN__vm_get_registor_value(
...
static bool VERMIN__vm_execute_instruction_JMP() {
  ...
  switch (jump_type) {
    case VERMIN_JMP_TYPE_JMP:
      jump_condition = true;
      break;
    case VERMIN_JMP_TYPE_JNZ:
      jump_condition = (registors[8] != 0);
      break;
    case VERMIN_JMP_TYPE_JZ:
      jump_condition = (registors[8] == 0);
      break;
    case VERMIN_JMP_TYPE_JGT:
      jump_condition = (registors[8] > 0);
      break;
    case VERMIN_JMP_TYPE_JLT:
      jump_condition = (registors[8] < 0);
      break;
    case VERMIN_JMP_TYPE_JGE:
      jump_condition = (registors[8] >= 0);
      break;
    case VERMIN_JMP_TYPE_JLE:
      jump_condition = (registors[8] <= 0);
      break;
    default:
  ...
  switch (jump_target_type) {
    case VERMIN_TARGET_CONST:
      jump_target = *(uint64_t*)(current_instruction.pointer + 1);
      break;
    case VERMIN_TARGET_REG:
      jump_target = (uint64_t)VERMIN__vm_get_registor_value(
          current_instruction.pointer[1]);
      break;
    case VERMIN_TARGET_REG_MEM:
      jump_target = (VERMIN__vm_as_number(
          &memory[VERMIN__vm_get_registor_value(
              current_instruction.pointer[2])],
          VERMIN__vm_set_registor_size(current_instruction.pointer[1])));
      break;
...
static bool VERMIN__vm_execute_instruction_PUSH() {
  ...
  switch (current_instruction.subparams) {
    case VERMIN_TARGET_REG:
      size = VERMIN__vm_set_registor_size(current_instruction.pointer[1]);
      value = VERMIN__vm_get_registor_value(current_instruction.pointer[1]);
      break;
    case VERMIN_TARGET_MEM:
      size = sizeof(uint64_t);
      value = (int64_t)(current_instruction.pointer[1]);
      break;
    case VERMIN_TARGET_CONST:
      size = sizeof(uint32_t);
      value = (int64_t)(*(int32_t*)(&current_instruction.pointer[1]));
      break;
    default:
...
static bool VERMIN__vm_execute_instruction_EXT() { return false; }
...
static bool VERMIN__vm_execute_instruction() {
  switch (current_instruction.id) {
    case VERMIN_INSTRUCTION_NOP:
      return true; /* VERMIN__vm_execute_instruction_NOP(); */
    case VERMIN_INSTRUCTION_MOV:
      return VERMIN__vm_execute_instruction_MOV();
    case VERMIN_INSTRUCTION_ADD:
      return VERMIN__vm_execute_instruction_ADD();
    case VERMIN_INSTRUCTION_SUB:
      return VERMIN__vm_execute_instruction_SUB();
    case VERMIN_INSTRUCTION_MUL:
      return VERMIN__vm_execute_instruction_MUL();
    case VERMIN_INSTRUCTION_DIV:
      return VERMIN__vm_execute_instruction_DIV();
    case VERMIN_INSTRUCTION_INC:
      return VERMIN__vm_execute_instruction_INC();
    case VERMIN_INSTRUCTION_DEC:
      return VERMIN__vm_execute_instruction_DEC();
    case VERMIN_INSTRUCTION_XOR:
      return VERMIN__vm_execute_instruction_XOR();
    case VERMIN_INSTRUCTION_JMP:
      return VERMIN__vm_execute_instruction_JMP();
    case VERMIN_INSTRUCTION_PUSH:
      return VERMIN__vm_execute_instruction_PUSH();
    case VERMIN_INSTRUCTION_POP:
      return VERMIN__vm_execute_instruction_POP();
    case VERMIN_INSTRUCTION_EXT:
      return VERMIN__vm_execute_instruction_EXT();
    case VERMIN_INSTRUCTION_SYSCALL:
      return VERMIN__vm_execute_instruction_SYSCALL();
    case VERMIN_INSTRUCTION_PRINTREG:
      return VERMIN__vm_execute_instruction_PRINTREG();
    case VERMIN_INSTRUCTION_AND:
      return VERMIN__vm_execute_instruction_AND();
    case VERMIN_INSTRUCTION_OR:
      return VERMIN__vm_execute_instruction_OR();
    case VERMIN_INSTRUCTION_LSHIFT:
      return VERMIN__vm_execute_instruction_LSHIFT();
    case VERMIN_INSTRUCTION_RSHIFT:
      return VERMIN__vm_execute_instruction_RSHIFT();
    default:
